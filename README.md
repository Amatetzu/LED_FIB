...
void loop() {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    if (bits[i]) {
      leds[i] = CRGB(redColor, greenColor, blueColor); // Set LED color to RGB values
    } else {
  ...
  delay(1000); // Adjust delay as needed for smooth animation
}
...
void pickColor() {
  ...
  int stepsize = third_max / 255; // Adjust as needed
